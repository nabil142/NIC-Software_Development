...
class _PolicyAnalystPageState extends ConsumerState<PolicyAnalystPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        final otherScenarios = scenarios.where((s) => s.id != active.id).toList();
  ...
  // Parse Markdown sections into structural data
  Map<String, List<String>> _parseAiOutput(String rawText) {
    ...
    for (var line in lines) {
      ...
      if (lower.startsWith('### rekomendasi terbaik')) {
        currentSection = 'rec_title'; // Just the name of the recommended scenario
        continue;
      ...
      // Add lines to the active section
    ...
    if (parsed['rec']!.isEmpty && parsed['risk']!.isEmpty && parsed['insight']!.isEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    // Map selected scenarios to datasets and colors
    final List<List<double>> datasets = [
      // Always include baseline as the first dataset
      [
    ...
    final List<Color> selectedColors = [
      const Color(0xFFEBB629) // Golden yellow for Baseline
    ];

    int colorIdx = 0;
    final List<ScenarioModel> selectedScenarios = [];

    // Add active scenario if selected
    ...
    // Fallback: if no scenario is selected, select active scenario
    if (selectedScenarios.isEmpty) {
      selectedScenarios.add(active);
      selectedColors.add(_distinctColors[0]);
    }

    // Add selected scenarios projected values to datasets
    ...
    final List<MapEntry<String, Color>> legendItems = [
      ...
      ...List.generate(selectedScenarios.length, (idx) {
        ...
        final color = selectedColors[idx + 1]; // Offset by 1 because index 0 is baseline color
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Selection chips for multi-scenario comparison
                  Text(
                  ...
                  Wrap(
                    ...
                    children: scenarios.map((s) {
                      final isSelected = _selectedScenarioIds.contains(s.id);
                      return FilterChip(
                        label: Text(s.scenarioName, style: GoogleFonts.inter(fontSize: 10)),
                        selected: isSelected,
                        ...
                        onSelected: (checked) {
                          setState(() {
                            ...
                            if (_carouselIndex >= selectedScenarios.length) {
                              _carouselIndex = 0;
                            }
                          });
                        },
                  ...
                  IntrinsicHeight(
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        // Card 1: Radar Chart + Legend (Symmetric Center Alignment)
                        Expanded(
                          child: Container(
                            decoration: BoxDecoration(
                              color: const Color(0xFFE9F0E6), // Sage green card background
                        ...
                        // Card 2: Progress Bars with centered chevron arrow on the right
                        Expanded(
                          child: Container(
                            decoration: BoxDecoration(
                              color: const Color(0xFFE9F0E6), // Sage green card background
                            ...
                            padding: const EdgeInsets.only(left: 16.0, top: 16.0, bottom: 16.0, right: 12.0),
                            child: Stack(
                              children: [
                                Padding(
                                  ...
                                  child: Builder(
                                    builder: (context) {
                                      final currentIdx = _carouselIndex % selectedScenarios.length;
                                ...
                                // Chevron right centered vertically on the far right edge of the card
                                Positioned(
                                  right: -8,
                                  top: 0,
                                  bottom: 0,
                                  child: Center(
                                    child: GestureDetector(
                                      onTap: () {
                                        setState(() {
                                          _carouselIndex = (_carouselIndex + 1) % selectedScenarios.length;
                                        });
                                      },
                                      child: const Icon(
                                        Icons.chevron_right,
                  ...
                  if (scenarioState.isLoading) ...[
                    ...
                    const Card(
                      child: Padding(
                        ...
                        child: Text('Gagal terhubung dengan AI. Silakan coba kembali.', textAlign: TextAlign.center),
                      ),
                    ),
                  ] else ...[
                    // Load parsed sections
                    _buildStructuredCards(active.scenarioName, active.aiAnalysis!),
                    const SizedBox(height: 20),

                    // Bottom Button "Lihat Blueprint"
                    ElevatedButton(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              // Icon Circle with border matching status color
              Container(
                width: 24,
                height: 24,
                decoration: BoxDecoration(
  ...
  Widget _buildHorizontalStructuredCard({
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Icon Container on the left with white icon
          Container(
            width: 56,
            height: 56,
            decoration: BoxDecoration(
  ...
  Widget _buildStructuredCards(String scenarioName, String rawText) {
    ...
    if (recBullets.isNotEmpty && recBullets[0].toLowerCase().contains('skenario')) {
    ...
    return Column(
      children: [
        // 1. YELLOW CARD: REKOMENDASI TERBAIK AI
        _buildHorizontalStructuredCard(
          cardBg: const Color(0xFFFFF4D4), // Soft warm yellow background
          iconBg: const Color(0xFFFBBF24), // Vibrant orange-yellow
            ...
            children: [
              if (recBullets.isNotEmpty) ...[
                Text(
                  ...
                  style: GoogleFonts.inter(fontSize: 11.5, color: AppTheme.textMedium, height: 1.4),
                ),
                const SizedBox(height: 10),
                ...recBullets.map((bullet) => Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4.0),
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Icon(Icons.check_circle, color: Color(0xFF6FAF4F), size: 16),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text(
                              bullet,
                              style: GoogleFonts.inter(fontSize: 11, color: AppTheme.textMedium, height: 1.35),
                            ),
                          ),
                        ],
                      ),
                ...
                const Text('AI memproyeksikan peningkatan seimbang di seluruh aspek lingkungan.', style: TextStyle(fontSize: 11)),
            ],
          ),
        ),
        
        // 2. RED CARD: RESIKO DAN HAMBATAN
        _buildHorizontalStructuredCard(
          cardBg: const Color(0xFFFFEBEB), // Soft pinkish-red background
          iconBg: const Color(0xFFF87171), // Vibrant red background
          ...
          content: Column(
            ...
            children: [
              if (riskBullets.isNotEmpty)
                ...riskBullets.map((bullet) => Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4.0),
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Container(
                            width: 6,
                            height: 6,
                            margin: const EdgeInsets.only(top: 6, left: 5, right: 10),
                ...
                const Text('Tidak ada resiko signifikan yang terdeteksi oleh AI.', style: TextStyle(fontSize: 11)),
            ],
          ),
        ),

        // 3. GREEN CARD: INSIGHT AI
        _buildHorizontalStructuredCard(
          cardBg: const Color(0xFFE9F5E6), // Soft green tint background
          iconBg: const Color(0xFF4C8C5A), // Vibrant green
          ...
          content: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...insightParagraphs.map((p) => Padding(
  ...
  Widget _buildNavItem(int index, IconData outline, IconData solid) {
    const int currentNavIndex = 2; // Active on AI Sustainability Analyst (Sparkles icon)
